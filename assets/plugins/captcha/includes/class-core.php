...
class Wc_Captcha_Core {
	...
	public function check_lost_password_with_captcha() {
		...
		if ( isset( $_POST['mc-value'] ) && $_POST['mc-value'] !== '' ) {
			if ( Wc_Captcha()->cookie_session->session_ids['default'] !== '' && get_transient( 'mc_' . Wc_Captcha()->cookie_session->session_ids['default'] ) !== false ) {
				if ( strcmp( get_transient( 'mc_' . Wc_Captcha()->cookie_session->session_ids['default'] ), sha1( AUTH_KEY . $_POST['mc-value'] . Wc_Captcha()->cookie_session->session_ids['default'], false ) ) !== 0 )
	...
	public function check_bbpress_captcha() {
		if ( isset( $_POST['mc-value'] ) && $_POST['mc-value'] !== '' ) {
			if ( Wc_Captcha()->cookie_session->session_ids['default'] !== '' && get_transient( 'bbp_' . Wc_Captcha()->cookie_session->session_ids['default'] ) !== false ) {
				if ( strcmp( get_transient( 'bbp_' . Wc_Captcha()->cookie_session->session_ids['default'] ), sha1( AUTH_KEY . $_POST['mc-value'] . Wc_Captcha()->cookie_session->session_ids['default'], false ) ) !== 0 )
